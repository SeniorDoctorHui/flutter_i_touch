...
class ColorSelect extends StatefulWidget {
  ...
  ColorSelect({this.colors, this.radius = 25, this.defaultColor, @required this.onColorSelect});
...
class _ColorSelectState extends State<ColorSelect> {
  ...
  Widget _buildItem(Color color) => Container(
      ...
      child: _getChildIndicator(color));

  Widget? _getChildIndicator(Color color) {
    if (activeColor == Colors.transparent && color == activeColor) {
      return _buildActiveTransparentIndicator();
  ...
  Widget _buildActiveTransparentIndicator() => Container(
